...
const ListItem = styled.div`
    ...
    &:hover {
        color: ${(props: {theme: {colors: string}}) => props.theme.colors.link};
    }
...
export class LanguageSwitcher extends React.Component<ILanguageSwitcherProps> {
    ...
    constructor(props: ILanguageSwitcherProps) {
        ...
        this.localeInfos = this.props.appConfig.getAvailableLocales().map((locale) => {
    ...
    private renderPopover() {
        return (
            <div style={{padding: '8px 16px'}} ref={(ref) => (this.layerEl = ref!)}>
                <div>
    ...
    private handleDocumentClick = (e: MouseEvent) => {
        if (
            !contains(this.layerEl, e.target as HTMLElement) &&
